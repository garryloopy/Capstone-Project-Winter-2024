...
const MenuPopUp = ({
  ...
  const PopUp = () => {
    return (
      <div
        data-show={showPopUp}
        className="fixed inset-0 backdrop-brightness-50 grid place-items-center z-50 opacity-0 pointer-events-none 
        data-[show=true]:pointer-events-auto data-[show=true]:opacity-100 transition-opacity duration-300"
        onClick={() => setShowPopUp(false)}
      >
        <div
          onClick={(ev) => ev.stopPropagation()}
          className="bg-gray-50 p-14 rounded-xl size-[40rem] overflow-y-scroll flex flex-col justify-center items-center gap-2"
        >
          {/* Image, title, desc section  */}
          <div className="w-full flex flex-col flex-1 items-center justify-center border py-8">
            <Image
              src={menuList.image}
              alt={menuList.title}
              width={200}
          ...
          {menuList.sizes?.length > 0 && (
            <div className="py-8 w-full flex flex-col border">
          ...
          {menuList.extra?.length > 0 && (
            <div className="py-8 w-full border flex flex-col">
